...
# 자격증명 작성 후 기동 (POSTGRES_PASSWORD·JWT_SECRET·SITES_YAML_PATH 등은 env로만 주입)
docker compose -f deploy/backend/docker-compose.yaml up -d --build
```

백엔드를 켜지 않아도 모든 검출은 CLI로 돌아간다 — `abloq scan freshness`, `abloq scan evidence`, `abloq scan cluster`, `abloq report monthly` 등. 한 인스턴스가 여러 사이트를 운용한다 — 사이트 목록은 `sites.yaml` 선언 SSOT(단일 사이트는 `BLOG_REPO_PATH`만으로 하위호환), 도메인 API는 `/sites/{site}/…` 스코프. blog.yaml과 abloqd manifest는 별개의 SSOT이며, abloqd는 마운트된 각 사이트 저장소의 blog.yaml을 읽어 언어·섹션·URL 계약과 GEO 임계를 파라미터로 쓴다.
...
도그푸드 #0(parkjunwoo.com 역이식) 검토에서 나온 채택 차단 요소와 운용 확장 요구를 묶은 마일스톤이다.
