...
curl -sf -X POST "$ABLOQD_URL/hooks/deployed" \
...
curl -s -m 600 -X POST "$ABLOQD_URL/archive/process" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"limit":200}' || echo "archive process deferred to the cron backstop"
```

③이 실패해도 배포는 성공으로 둔다 — pending 영수증은 cron 백스톱
(abloqd compose의 `archiver-backstop`, 권고 주기 15분: login → retry →
process)이 처리한다. 결과 확인은 `GET /receipts?deploy_id=<id>`.
