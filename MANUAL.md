...
### 이미지 (순수 Go — cgo·외부 바이너리 없음)

| 명령 | 인자·플래그 | 의미 |
|---|---|---|
| `abloq image og <slug> <title>` | `--brand --font --out` | 1200×630 OG WebP를 `static/images/{slug}.webp`에 생성 |
...
> 환경 변수: `archive`·`ingest`·`sample`은 자격증명을 **env로만** 받는다 —
> `AWS_*`(S3 로그), `GSC_SA_JSON`/`GSC_SA_JSON_PATH`(GSC), IndexNow/Wayback 키.
...
- `abloq image og`: `--provider`(local 기본 | gemini)·`--model`·`--overlay`·`--variant`·`--all-variants`·`--count` 추가. blog.yaml `image.og`에 공통 프롬프트·모델·안(variant) 프리셋 선언. 다중 안은 `files/og/{slug}/`에 후보 산출 → 검토 → `mv` 채택. API 키는 env 전용(`GEMINI_API_KEY`).
