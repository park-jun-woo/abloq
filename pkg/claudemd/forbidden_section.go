...
//ff:what CLAUDE.md 금지 사항 섹션 — 치즈 방어 원칙: 게이트가 잡는 모든 우회를 시도 자체부터 금지, llms.txt는 mode auto일 때만 손수정 금지 목록에 포함
package claudemd

import "github.com/park-jun-woo/abloq/pkg/blogyaml"

// forbiddenSection renders the cheese-defense rules. Every entry mirrors a
// deterministic gate — bypassing is detected, so don't attempt it. llms.txt
// joins the no-hand-edit list only in llms_txt mode auto.
func forbiddenSection(b *blogyaml.Blog) string {
	derived := "hugo.toml/robots.txt/jsonld.json"
	if b.Geo.LlmsTxtMode() == "auto" {
		derived = "hugo.toml/robots.txt/llms.txt/jsonld.json"
	}
	return `## 금지 사항 (치즈 방어)

게이트는 결과를 잡지만, 시도 자체가 금지다. 게이트를 "통과시키는" 게 아니라 일을 끝내라.

- 생성물(` + derived + `)을 손으로 고치지 않는다 — blog.yaml을 고치고 ` + "`abloq generate`" + `.
