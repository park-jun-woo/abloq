...
//ff:what GSC 그룹 처리 — 사이트 자격(Keys)으로 토큰 교환, 신규(date==lastmod) 우선 정렬 → 쿼터 상한까지 제출·초과분 deferred → SA 토큰 실패 시 전 항목 failed
...
func processGSC(keys Keys, pending []Pending) []Item {
	if len(pending) == 0 {
		return nil
	}
	...
	token, err := GSCTokenWith(keys, ScopeIndexing)
	if err != nil {
