...
func TestScan(t *testing.T) {
	...
	items := Scan(entries, map[string]priority.Signals{}, []string{"ko"}, 1, now, priority.ColdStart{})
	if len(items) != 1 {
	...
	if len(it.Keys) != 1 || it.Keys[0] != "ko/tech/stale-a" {
		t.Errorf("keys must carry the declared-language join keys: %v", it.Keys)
	}
	// Crawl hits override the date fallback.
	signals := map[string]priority.Signals{"ko/tech/stale-a": {Hits: 42}}
	items = Scan(entries, signals, []string{"ko"}, 1, now, priority.ColdStart{})
