...
//ff:what 신선도 초과 글 1건 → refresh 큐 후보 — 신호 맵에서 측정 신호를 꺼내 date를 채워 스코어러에 전달, keys는 전 언어, payload에 근거(lastmod·threshold)만 (now-파생값 금지)
package freshness
...
// fallback stays available; Keys lists the join key per declared language
// (Phase018 consumer contract). The payload holds only the generation
// rationale (lastmod + threshold) — never a now-derived value, so re-scans
// serialize byte-identically.
func candidate(e content.Entry, signals map[string]priority.Signals, langs []string, days int, scorer priority.Scorer) queueio.Item {
	...
	return queueio.Item{
		...
		Keys:     queueio.KeysFor(langs, e.Section, e.Slug),
