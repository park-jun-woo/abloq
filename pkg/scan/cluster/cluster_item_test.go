...
//ff:what clusterItem이 kind=cluster 항목을 결정적 payload(JSON 문자열 값)와 위반+후보 합 priority로 조립하는지 검증
package cluster
...
func TestClusterItem(t *testing.T) {
	...
	it := clusterItem(p, "ko", viols, cands)
	...
	empty := clusterItem(p, "ko", viols, nil)
