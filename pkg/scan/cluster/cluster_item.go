...
//ff:what 글 1편 → kind=cluster 큐 항목 — payload에 violations(JSON)·candidates(JSON), keys는 전 언어, priority는 위반+후보 건수 합
...
func clusterItem(p post, lang string, viols []Violation, cands []Candidate, langs []string) queueio.Item {
	...
	return queueio.Item{
		...
		Keys:     queueio.KeysFor(langs, p.Section, p.Slug),
		Payload:  payload,
	}
}
