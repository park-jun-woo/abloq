...
//ff:what 글 1편 → kind=evidence 큐 항목 — payload에 claims(JSON)·rot_urls(JSON), keys는 전 언어, priority는 검출 건수 합
...
// order keeps the bytes deterministic); Keys lists the join key per declared
// language (Phase018 consumer contract); priority is the finding count — the
// more decay, the sooner the agent should look.
func evidenceItem(a *gate.Article, claims []ClaimRef, rots []string, langs []string) queueio.Item {
	...
	return queueio.Item{
		...
		Keys:     queueio.KeysFor(langs, a.Section, a.Slug),
		Payload:  payload,
	}
}
