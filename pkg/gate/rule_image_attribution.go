...
//ff:what [image-attribution] structure.order에 attribution이 선언된 경우 메인 이미지 직후 이탤릭 저작자 표기 검증 — layout 특수 페이지는 스킵
package gate
...
// the dependency (image-first names the root cause). Layout-owned special
// pages are exempt (see special).
func ruleImageAttribution(t *Target) []blogyaml.Diagnostic {
	...
	for _, a := range t.Articles {
		if (a.Doc.FirstIsImage && a.Doc.AttribLine >= 0) || special(a) {
			continue
		}
