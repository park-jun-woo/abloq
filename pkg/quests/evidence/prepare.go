...
//ff:what Prepare — 제출 JSON 디코드, 대상 글 일치 검증, 공통 Consumption 조립 + payload claims 해시·rot URL 적재
...
func (Definition) Prepare(_ *quest.Session, it *quest.Item, raw []byte) (rgate.Context, *quest.Verdict, error) {
	...
	cons.QueuedClaims, err = queuedClaims(p.Queue)
	if err != nil {
		return rgate.Context{}, nil, err
	}
	rots, err := queuedRots(p.Queue)
	if err != nil {
		return rgate.Context{}, nil, err
	}
	sub := &Submission{Consumption: cons, Article: p.Article, RotURLs: rots}
