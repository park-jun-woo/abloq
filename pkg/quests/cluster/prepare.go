...
//ff:what Prepare — 제출 JSON 디코드, 대상 글 일치 검증, 공통 Consumption 조립 + payload 위반 룰 적재 + candidates 경로로 허용 집합 확장
...
// repository, malformed frozen payload) abort the submit without burning a
// try. No short verdict is ever returned (nothing is untrusted here).
func (Definition) Prepare(_ *quest.Session, it *quest.Item, raw []byte) (rgate.Context, *quest.Verdict, error) {
	...
	viols, err := queuedViolations(p.Queue)
	if err != nil {
		return rgate.Context{}, nil, err
	}
	paths, err := candidatePaths(p.Queue, p.Lang)
	if err != nil {
		return rgate.Context{}, nil, err
	}
	for _, path := range paths {
		cons.Allowed[path] = true
	}
	sub := &Submission{Consumption: cons, Article: p.Article,
		Lang: p.Lang, Section: p.Section, Slug: p.Slug, ViolRules: viols}
